...
func parseConfig(json gjson.Result, config *WafConfig, log wrapper.Log) error {
	...
	conf := coraza.NewWAFConfig().WithRootFS(coraza_tools.Root)
...
func onHttpRequestHeaders(ctx wrapper.HttpContext, config WafConfig, log wrapper.Log) types.Action {
	...
	srcIP, srcPort := coraza_tools.RetrieveAddressInfo(log, "source")
	dstIP, dstPort := coraza_tools.RetrieveAddressInfo(log, "destination")
	...
	if err == nil {
		...
		tx.SetServerName(coraza_tools.ParseServerName(log, authority))
	}

	interruption := tx.ProcessRequestHeaders()
	if interruption != nil {
		return coraza_tools.HandleInterruption(ctx, "http_request_headers", interruption, log)
	}
...
func onHttpRequestBody(ctx wrapper.HttpContext, config WafConfig, body []byte, log wrapper.Log) types.Action {
	...
	if !tx.IsRequestBodyAccessible() {
		...
		if interruption != nil {
			return coraza_tools.HandleInterruption(ctx, "http_request_body", interruption, log)
		}
...
func onHttpResponseHeaders(ctx wrapper.HttpContext, config WafConfig, log wrapper.Log) types.Action {
	...
	if !ctx.GetContext("processedRequestBody").(bool) {
		...
		if interruption != nil {
			return coraza_tools.HandleInterruption(ctx, "http_response_headers", interruption, log)
		}
	}
...
func onHttpResponseBody(ctx wrapper.HttpContext, config WafConfig, body []byte, log wrapper.Log) types.Action {
	...
	if ctx.GetContext("interruptionHandled").(bool) {
		...
		return coraza_tools.ReplaceResponseBodyWhenInterrupted(log, coraza_tools.ReplaceResponseBody)
	}
	...
	if !tx.IsResponseBodyAccessible() {
		...
		if interruption != nil {
			...
			return coraza_tools.HandleInterruption(ctx, "http_response_body", interruption, log)
		}
		return types.ActionContinue
	}
