...
func initWaf() {
	...
	if err != nil {
		proxywasm.LogErrorf("Failed to create waf conf: %v", err)
	}

	WAF = waf
	proxywasm.LogInfo("Create waf conf succeed!")
}
...
func onHttpRequestHeaders(ctx wrapper.HttpContext, config WafConfig, log wrapper.Log) types.Action {
	...
	//log.Info("Start processing headers.")
	if ctx == nil {
		log.Error("Sorry, ctx is nil")
	}
	//log.Info("Set interruptionHandled.")
	ctx.SetContext("interruptionHandled", false)
	//log.Info("Set processedRequestBody.")
	ctx.SetContext("processedRequestBody", false)
	//log.Info("Set processedResponseBody.")
	ctx.SetContext("processedResponseBody", false)
	//log.Info("Set tx.")
	ctx.SetContext("tx", WAF.NewTransaction())
	//log.Info("Get tx.")
