...
const (
	Ptn954100 = `[a-z]:\x5cinetpub\b`
	Ptn954110 = `(?:Microsoft OLE DB Provider for SQL Server(?:</font>.{1,20}?error '800(?:04005|40e31)'.{1,40}?Timeout expired| \(0x80040e31\)<br>Timeout expired<br>)|<h1>internal server error</h1>.*?<h2>part of the server has crashed or it has a configuration error\.</h2>|cannot connect to the server: timed out)`
	Ptn954130 = `\bServer Error in.{0,50}?\bApplication\b`
)

var Re954100, Re954110, Re954130 *re2.Regexp

var IIS_ERRORS = []string{
...
var Rule954120Matcher ahocorasick.AhoCorasick

func init() {
	Re954100, _ = re2.Compile(Ptn954100)
	Re954110, _ = re2.Compile(Ptn954110)
	Re954130, _ = re2.Compile(Ptn954130)

	Rule954120Matcher = AHO_CORASICK_BUILDER.Build(IIS_ERRORS)
}
