...
func CmdLineRce(data string) (string, bool, error) {
	for i := 0; i < len(data); i++ {
		if needsTransformRce(data[i]) {
			return doCMDLineRce(data, i), true, nil
		}
	}
	return data, false, nil
}

func doCMDLineRce(input string, pos int) string {
...
func needsTransformRce(c byte) bool {
	if c >= 'A' && c <= 'Z' {
		return true
	}
	switch c {
