...
func doJsDecode(input string, pos int) (string, bool) {
	...
	for i < inputLen {
		if input[i] == '\\' {
			...
			case (i+5 < inputLen) && (input[i+1] == 'u') && (ValidHex(input[i+2])) && (ValidHex(input[i+3])) && (ValidHex(input[i+4])) && (ValidHex(input[i+5])):
				/* \uHHHH */
				...
				c++
				i += 6
			case (i+3 < inputLen) && (input[i+1] == 'x') && ValidHex(input[i+2]) && ValidHex(input[i+3]):
				/* \xHH */
				d[c] = X2c(input[i+2:])
				changed = true
				c++
