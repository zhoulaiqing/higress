...
const (
	Ptn953110 = `(?:\b(?:f(?:tp_(?:nb_)?f?(?:ge|pu)t|get(?:s?s|c)|scanf|write|open|read)|gz(?:(?:encod|writ)e|compress|open|read)|s(?:ession_start|candir)|read(?:(?:gz)?file|dir)|move_uploaded_file|(?:proc_|bz)open|call_user_func)|\$_(?:(?:pos|ge)t|session))\b`
	Ptn953120 = `(?i)<\?(?:=|php)?\s+`
)

var Re953110, Re953120 *re2.Regexp

var PHP_ERRORS = []string{
	...
	"Bit field \"",
	"Breakpoint exists at Breakpoint exists for opline C array index out of bounds",
	...
	"Failed to find breakpoint Failed to read property due to libxml error",
	...
	"Only classes can be marked with Only the first byte will be assigned to the string offset",
	...
	"and argument and argument args element is not an array",
	...
	"cannot be null when argument cannot be null when argument cannot be null when argument cannot be null when argument cannot be null when the socket type is AF_INET",
	"cannot be null when the socket type is AF_INET6",
	"cannot be provided when argument cannot be true when argument cannot change a protected metatable",
	"cannot combine mode \"",
	...
	"must be a multiple of argument must be a non-empty string",
	...
	"must be between 1 and 65535 when argument must be between 1 and 7",
	"must be between 1 and the number of elements in argument must be between 1970 and 2037 (inclusive)",
	"must be between 2 and 36 (inclusive)",
	"must be contained in argument must be contained in argument must be contained in argument must be either COUNT_NORMAL or COUNT_RECURSIVE",
	...
	"must be greater than or equal to argument must be in the range of 0-255",
	"must be integer-indexed",
	"must be less than 65535 bytes",
	"must be less than argument must be less than or equal to 1048576",
	"must be less than or equal to INT_MAX - 4 bytes",
	"must be less than or equal to SODIUM_CRYPTO_KDF_BYTES_MAX",
	"must be less than or equal to argument must be less than or equal to the length of argument must be less than the number of fields for this result set",
	...
	"must be null when argument must be null when argument must be null when argument must be null when argument must be odd if argument must be of type PgSql\\Connection when the connection is provided",
	"must be of type array when argument must be of type array when using translate or scale",
	"must be of type array, string given",
	"must be of type string when argument must be of type string, array given",
	...
	"must be provided when argument must be specified when enabling encryption",
	"must be the same size as argument must contain arrays only containing the \"",
	...
	"when argument with an empty name",
	"' (include_path='",
	"' (info)\n",
	"' (mem)\n",
	"' (tried:",
	...
	"() to object of class",
	"(): Argument (): Argument (): Argument (): Argument (): Array value for VLV control must have a \"",
	...
	"(): Expects exactly 3 arguments when argument (): If option \"",
	"(): Option must be a valid callback",
	"(): Parameter (): Return type must be",
	...
	"No frame No help topic found for",
	...
	"array_key_exists(): Argument avif error -",
	"bad argument bad magic in `",
	"call_user_func_array(): Argument cannot allocate %",
	"cannot find entry `",
	"cannot mprotect `",
	...
	"get_class(): Argument getcwd() failed for '",
	"getifaddrs() failed",
	...
	"must be less than or equal to argument must be of type ?",
	...
	"stack overflow (",
	"string too long: `",
	"strlen(): Argument unreadable symlink `",
	"upload_max_filesize of",
...
var Rule953100Matcher ahocorasick.AhoCorasick

func init() {
	Rule953100Matcher = AHO_CORASICK_BUILDER.Build(PHP_ERRORS)

	Re953110, _ = re2.Compile(Ptn953110)
	Re953120, _ = re2.Compile(Ptn953120)
}
